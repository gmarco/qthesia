        ...
        while ( next_event_time <= pretend_clock_time )
        {
            ...
            if ( seq->GetNextEvent ( &ev_track, &ev ) )
            {
				...
				if (msg->GetLength()>0){
					...
					//if ((msg->GetStatus()&0xe0)==0x80){
						mess.push_back(msg->GetStatus());
						if (msg->GetLength()>0) mess.push_back(msg->GetByte1());
						if (msg->GetLength()>1) mess.push_back(msg->GetByte2());
						if (msg->GetLength()>2) mess.push_back(msg->GetByte3());
						if (msg->GetLength()>3) mess.push_back(msg->GetByte4());
						if (msg->GetLength()>4) mess.push_back(msg->GetByte5());
						mess.resize(msg->GetLength());
...
int main( int argc, char **argv )
{
    if ( argc > 1 || 1 )
    {
		...
		infile_name="/Users/marco/lady_gaga-poker_face.mid";
